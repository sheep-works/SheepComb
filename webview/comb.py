...
def render_results(segments, is_matched=False):
    ...
    # 【JS開発用】最新の全セグメントデータを JS のグローバル変数に共有
    # これにより、JS 側から window.allSegments でデータにアクセスできるようになります
    js.window.allSegments = to_js(segments)
